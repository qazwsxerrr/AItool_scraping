...
GitHub Trending/Search repository 已经包含结构化 stars、周期新增 Star、forks、language、topics 和 pushed_at（Search）。处理阶段只执行 registry 规则，直接标记 `hotspot` 并写入 metadata verification，不调用 AI；导出后由 `app/storage/github_reader.py` 按 Trending 周期指标和 metadata 排序。
...
`app/collectors/unified.py` 中的 RSS/Atom、RSSHub、GitHub API、GitHub Trending HTML 和 Product Hunt collector 只负责请求和字段映射，接受共享 `httpx.Client`，返回 `FetchBatch`；数据库写入、筛选、AI 和核实均在 Job/Domain 层完成。
