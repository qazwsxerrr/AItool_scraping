...
def test_generic_json_posts_item_and_normalizes_response():
    ...
    assert http.calls[0]["json"]["task"] == "ai_item_analysis"
...
def test_openai_chat_and_responses_envelopes_are_supported():
    ...
    assert response_client.analyze(_request(source_content_class="official_model_company")).summary_cn == "Responses API 社区线索"
...
def test_malformed_or_incomplete_provider_json_is_rejected():
    ...
    incomplete = FakeHttpClient(FakeResponse({"keep": True, "content_class": "project_tool", "reason": "incomplete", "risk_flags": [], "confidence": 50}))
    with pytest.raises(ValueError, match="missing required fields"):
        ItemAnalysisClient(api_url="https://ai.example.test", api_key="key", http_client=incomplete).analyze(_request())
