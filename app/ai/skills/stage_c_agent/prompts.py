"""Prompt and local function schemas for the Stage-C plan/commit agent."""
...
STAGE_C_AGENT_PROMPT_VERSION = "stage_c_agent_v16"
...
- 工作方式是 Plan/Commit：用只读工具和研究工具收集证据，最后一次 submit_event_plan 提交覆盖全部 active 的完整事件方案。本地负责校验、强身份归一和原子落库。不要分段持久化草稿，也不要多轮挪动已落库成员。
...
- 若必须拆分同一 event_family_key 下的多个 candidate/needs_review 事件，每个事件都必须填写 split_reason。只允许这些值：different_model_or_major_version、separate_time_window_actionable、independent_security_policy_or_breaking_change、platform_released_independent_product、standalone_pricing_quota_access_change。常规“上线多个平台”“某平台给了折扣”“媒体补充能力和价格”不是充分拆分理由。
- event_family_key 是同一日报事件包的稳定短码。它用于本地校验和 Stage D 去刷屏，不是分类标签。
- event_key 是本次方案内事件的稳定键；搜索证据也按 event_key 绑定。
...
- facts 是事件包的事实清单；每条 fact 必须有 supporting_item_ids，且只能引用本事件成员。candidate 必须至少有一条 fact；needs_review 或 rejected 可为空，但不得编造事实补齐。
...
- 历史事件如有实质增量，把增量事实写入 facts，并使用 history_status=meaningful_update。
...
- 对聚合歧义、弱来源实质更新、来源冲突或关键事实不确定，先在完整方案里把该事件标为 publishability=needs_review，再调用 search_web。搜索前明确缺失的事件核心证据，query 和 claim 必须核验可能改变 publishability 的关键事实，不能只重复确认文章、转载或原话存在。搜索结果必须通过 attach_search_evidence 按 result_id 绑定到 event_key 和具体 claim；不得自行提供搜索结果之外的 URL。
...
- 标题使用简洁中文，必须是“明确主体 + 明确动作/发现/变化”的事件标题；summary_cn 再补充限定口径和证据边界。通过工具完成工作；submit_event_plan 若返回校验错误，根据 errors 修正后重新提交完整方案，不要只提交局部补丁。
</working_principles>
""".strip()
...
LIST_PLAN_SNAPSHOT_SCHEMA = object_schema({}, [])
...
SUBMIT_EVENT_PLAN_SCHEMA = object_schema(
    {
        "events": {
            "type": "array",
            "minItems": 1,
            "maxItems": 80,
            "items": object_schema(EVENT_PLAN_ITEM_PROPERTIES, EVENT_PLAN_ITEM_REQUIRED),
        }
    },
    ["events"],
)
